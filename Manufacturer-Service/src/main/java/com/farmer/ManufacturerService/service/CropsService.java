...
public class CropsService {
    ...
    public Crops updateCrop(int manufacterId,Crops crops) {
        Crops crop=new Crops();
        Optional<Crops> updateCrop=cropsRepository.findById(crops.getCropId());
        if(updateCrop.isPresent()){
            ...
            crop.setManufacturer(manurepo.findById(manufacterId).get());
        }else{
            return new Crops();
        }
        return  cropsRepository.save(crops);
